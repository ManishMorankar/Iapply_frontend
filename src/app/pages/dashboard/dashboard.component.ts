...
export class DashboardComponent implements OnInit {
  ...
  constructor(public dashboardService: DashboardService, private router: Router, private apiService: ApiService) {
    this.trends.map(t => {
      t.data = this.getRandomData();
    });

    this. dashboardService.onReceiveDataSuccess.subscribe(event => {
      if (event) {
        this.initChart();
      }
    });
  ...
  initChart() {
    jQuery.plot($(this.chartContainer.nativeElement), this.dashboardService.revenue, {
      series: {
        pie: {
          innerRadius: 0.8,
          show: true,
  ...
  ngOnInit() {
    ...
    this.dashboardService.receiveDataRequest();
  }
}
