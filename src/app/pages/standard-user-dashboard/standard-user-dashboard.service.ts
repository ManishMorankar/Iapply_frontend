...
export class StandardUserDashboardService {
  _visits: any = {};
  ...
  receiveDataRequest() {
    ...
    if (!this.config.isBackend) {
      ...
      this.http.get('/standard-user-dashboard').subscribe(data => {
        this.receiveDataSuccess(data);
      });
    }
  }
  ...
  get mainChart1() {
    return this.mainChart1;
  }

  set mainChart1(mainChart1) {
    this._mainChart = mainChart1;
  }
